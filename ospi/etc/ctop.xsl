...
Modified by Kyle Siegrist, 2004/11/23
-->
...
<xsl:call-template name="infix">
 ...
 <xsl:with-param name="p" select="$p"/>
 <xsl:with-param name="mo"><mml:mo>&#8290;<!-- invisible times --></mml:mo></xsl:with-param>
</xsl:call-template>
</xsl:template>
...
<xsl:template mode="c2p" match="mml:apply[*[1][self::mml:diff]]" priority="1">
 ...
 <mml:mrow><mml:msup><mml:mo mathvariant="italic">d</mml:mo><xsl:apply-templates mode="c2p" select="mml:bvar/mml:degree/node()"/></mml:msup>
     <xsl:apply-templates mode="c2p"  select="*[last()]"/></mml:mrow>
 <mml:mrow><mml:mo mathvariant="italic">d</mml:mo><mml:msup><xsl:apply-templates mode="c2p"
 select="mml:bvar/node()"/><xsl:apply-templates mode="c2p"
 select="mml:bvar/mml:degree/node()"/></mml:msup>
</mml:mrow>
</xsl:when>
<xsl:otherwise>
 <mml:mrow><mml:mo mathvariant="italic">d</mml:mo><xsl:apply-templates mode="c2p" select="*[last()]"/></mml:mrow>
 <mml:mrow><mml:mo mathvariant="italic">d</mml:mo><xsl:apply-templates mode="c2p" select="mml:bvar"/></mml:mrow>
</xsl:otherwise>
 </xsl:choose>
 </mml:mfrac>
</xsl:template>
...
<mml:mrow>
<mml:mi><xsl:value-of select="local-name(*[1])"/></mml:mi><mml:mo>(</mml:mo>
<xsl:apply-templates mode="c2p" select="*[2]">
...
<xsl:template mode="c2p" match="mml:pi">
  <mml:mi mathvariant="normal">&#960;<!-- pi --></mml:mi>
</xsl:template>
...
<xsl:template mode="c2p" match="mml:infinity">
  <mml:mi mathvariant="normal">&#8734;<!-- infinity --></mml:mi>
</xsl:template>
